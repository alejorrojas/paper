...
def main() -> None:
    ...
    add_p(
        doc,
        (
            "La motivación de este trabajo es una necesidad identificada en nuestras "
            "propias rondas de experimentación con Natural Language Autoencoders (NLA) "
            "sobre las herramientas disponibles. Neuronpedia permite inspeccionar un "
            "chat y un token a la vez, pero no cubre el ciclo de evaluación de un "
            "experimento. Ese vacío contrasta con las aplicaciones basadas en LLM, "
            "donde frameworks como LangSmith son estándar para datasets, evaluadores y "
            "comparación de corridas. Un NLA comprime el vector de activación de un "
            "modelo de lenguaje en un texto breve y reconstruye ese vector a partir "
            ...
            "modelo. Anthropic aplica un LLM como juez sobre AVs como método interno "
            "de un paper, no como herramienta reutilizable. El presente trabajo "
            "presenta NLA Eval, una plataforma que operacionaliza el ciclo de "
            "experimento sobre NLA públicos mediante datasets de prompts, una "
            "política de tokens, llamadas a la API de Neuronpedia, evaluadores "
            "configurables y una vista de comparación de corridas. El objeto "
            "evaluado es el AV en una posición de token, no la respuesta pública del "
            "modelo. La validación empírica en contextos de investigación y el "
            "acuerdo humano-juez sobre un corpus etiquetado constituyen los pasos "
    ...
    add_p(
        doc,
        "Natural Language Autoencoders, interpretabilidad, LLM-as-judge, Neuronpedia, LangSmith, evaluación de modelos de lenguaje",
        10,
        space_after=12,
    )
